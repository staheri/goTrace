...
vector<Entry> readEntryFile(string path){
	...
	vector<string> tmpVst;
	vector<string>::iterator vst_it2;
	Entry tmp;
	entries.clear();
	string line,dtabPath,ltabPath;
	ifstream fi(path);

	if(fi.is_open()){
		while(getline(fi,line)){
			tmp = Entry();
			printf("READ_ENTRY line >: %s\n",line.c_str() );
			tmpVst = splitString(line,'^');
			if (tmpVst.size() > 1){
				tmp.addElement(splitString(splitString(tmpVst[0],')')[0],'(').back());
				tmp.setLC(stoi(tmpVst.back()));
			}else{
				tmp.addElement(tmpVst.back());
				tmp.setLC(1);
			}
			//printf("THE ENTRY >: %s\n",tmp.toString().c_str() );
	...
	tmpVst = splitString(path,'/');
	// Figuring out dtab Path
	dtabPath = "";
	for (vst_it2 = tmpVst.begin() ; vst_it2 != tmpVst.end() -1; vst_it2++){
		dtabPath = dtabPath + *vst_it2 + "/";
	}
	dtabPath = dtabPath + "dtab.txt";

	// Read dtab from file
	ifstream fid(dtabPath);
	if(fid.good()){
		if(fid.is_open()){
			while(getline(fid,line)){
				entries[0].loadDtab(line);
			}
		}else{
			printf("I/O Error\nUnable to open/read DTAB %s\n",dtabPath.c_str());
		}
	}

	// Figuring out ltab Path
	ltabPath = "";
	for (vst_it2 = tmpVst.begin() ; vst_it2 != tmpVst.end() -1; vst_it2++){
		ltabPath = ltabPath + *vst_it2 + "/";
	}
	ltabPath = ltabPath + "ltab.txt";

	// Read ltab from file
	ifstream fil(ltabPath);
	if(fil.good()){
		if(fil.is_open()){
			while(getline(fil,line)){
				entries[0].loadLtab(line);
			}
		}else{
			printf("I/O Error\nUnable to open/read LTAB %s\n",ltabPath.c_str());
		}
	}
	return entries;
}
